...
/******************************************************************************
PURPOSE: CeilometerSubset.c - Read a set of a Ceilometer files, subset the
...
HISTORY: 2011-07-28 plessel.todd@epa.gov
...
#include "ReadData.h"  /* For readSubsetCeilometerData(). */
...
static Integer isValidElevationRange( const Real elevationRange[ 2 ] ) {
  ...
  AND3( isNanFree( elevationRange, 2 ),
        IN_RANGE( elevationRange[ MINIMUM ], -500.0, 100000.0 ),
        IN_RANGE( elevationRange[ MAXIMUM ],
                  elevationRange[ MINIMUM ], 100000.0 ) );
...
static Integer isValidProfile( const Profile* profile ) {
  Integer result =
    AND13( profile,
           ...
           IN_RANGE( profile->elevation, -500.0, 100000.0 ),
           profile->id > 0,
           profile->units[ 0 ],
           profile->notes[ 0 ] );
  ...
  if ( result ) {
    ...
    const Real minimum = profile->units[ 0 ] == 'm' ? -500.0 : 0.0;
    const Real maximum = profile->units[ 0 ] == 'm' ? 100000.0 : 2e6;
    Integer point = 0;
    ...
    for ( point = 0; AND2( result, point < points ); ++point ) {
      ...
      result = AND2( result, IN_RANGE( elevation, -500.0, 100000.0 ) );
    }
  }
...
static void printUsage( const char* programName ) {
  ...
  fprintf( stderr, "  -variable blview_backscatter_profile \\\n" );
  ...
  fprintf( stderr, "-desc "
           "http://www.vaisala.com/en/products/ceilometers/Pages/CL51.aspx,"
           "CeilometerSubset \\\n" );
  fprintf( stderr, "-time 20140715000000 20140715235959 \\\n" );
  fprintf( stderr, "-variable blview_backscatter_profile \\\n" );
...
static void initializeArguments( Arguments* arguments ) {
  ...
  arguments->elevationRange[ MAXIMUM ] = 100000.0;
}
...
static Integer parsedElevationRange( Integer argc, char* argv[], Integer* arg,
  ...
  POST02( IS_BOOL( result ),
          IMPLIES_ELSE( result,
                        AND3( IN_RANGE( range[ MINIMUM ], -500.0, 100000.0 ),
                              IN_RANGE( range[ MAXIMUM ],
                                        range[ MINIMUM ], 100000.0 ),
                              *arg == OLD( arg ) + 3 ),
...
INPUTS:  const char* fileName  Name of a Ceilometer file.
                               discoveraq-cl51_GROUND-BAO-TOWER_20140715_R0.h5
...
static Integer instrumentId( const char* fileName ) {
  PRE03( fileName, *fileName, strlen( fileName ) > 14 );
  Integer result = 0;
  const Integer length = strlen( fileName );
  const Integer end = length - 14; /* Before timestamp. */
  Integer index = 0;
