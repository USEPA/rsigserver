...
#if 0
/* plessel.todd@epa.gov 2017-08-03 replaced this nice tail-recursive routine
 * with a non-recursive routine below to speed-up in my application. */
...
static edge_node **bound_list(lmt_node **lmt, double y)
{
  ...
  do
  {
    lmt_node* const current_node = *lmt;
    ...
    {
      const double current_node_y = current_node->y;
      ...
      if (y < current_node_y)
      {
        ...
        (*lmt)->y= y;
        (*lmt)->first_bound= NULL;
        (*lmt)->next= current_node;
        result = &((*lmt)->first_bound);
      }
      else if (y > current_node_y)
        lmt = &(current_node->next); /* Head further up the LMT */
      else
        result = &((*lmt)->first_bound); /* Use this existing LMT node */
    }
  } while ( result == 0 );
...
#if 0
/* plessel.todd@epa.gov 2014-11-20 replaced this nice tail-recursive routine
with a non-recursive routine below to achieve 4x speed-up in my application. */
...
#else
/* This non-recursive routine is less elegant but 4x faster. */
static void add_to_sbtree(int* const entries, sb_tree **sbtree, const double y)
{
  int done = 0;
  do
  {
    sb_tree* ssbtree = *sbtree;
    if (ssbtree == 0)
    {
      /* Add a new tree node here */
      MALLOC(*sbtree, sizeof(sb_tree), "scanbeam tree insertion", sb_tree);
    ...
    {
      const double ssbtree_y = ssbtree->y;
      if (ssbtree_y > y)
        sbtree = &(ssbtree->less);
      else if (ssbtree_y < y)
        sbtree = &(ssbtree->more);
...
#if 0
/* plessel.todd@epa.gov 2017-08-03 replaced this nice tail-recursive routine
 * with a non-recursive routine below to speed-up my application. */
...
{
  while (*tn)
    tn = &((*tn)->next); /* Head further down the list */
                      ...
                      gpc_polygon *result)
{
  ...
  h_state        horiz[2]={0,0};
  int            in[2]={0,0}, exists[2]={0,0}, parity[2]= {LEFT, LEFT};
  int            c=0, v=0, contributing=0, search=0, scanbeam= 0, sbt_entries= 0;
  int            vclass=0, bl=0, br=0, tl=0, tr=0;
  double        *sbt= NULL, xb=0, px=0, yb=0, yt=0, dy=0, ix=0, iy=0;
                       ...
                       gpc_tristrip *result)
{
  ...
  lmt_node      *lmt= NULL, *local_min=0;
  polygon_node  *tlist= NULL, *tn=0, *tnn=0, *p=0, *q=0;
