...
/******************************************************************************
PURPOSE: BEHRSubset.c - Read a set of a OMI-BEHR files, subset the swath to a
...
typedef struct {
  const char* listFile;       /* File containing list of BEHR files to read*/
...
typedef struct {
  ...
  Integer points;    /* Number points in domain-subsetted filtered swath.*/
} SwathInfo;
...
/* Bounds file contents: */

typedef struct {
  FileName fileName;     /* Pathed file name. */
  Real bounds[ 2 ][ 2 ]; /* bounds[ LONGITUDE, LATITUDE][ MINIMUM, MAXIMUM ] */
} FileBounds;
...
#define TEMP_FILE_NAME "junk_BEHRSubset"
...
static FileBounds* readListFile( const char* listFile, Integer* fileCount );

static Integer convertNewlines( const char* boundsFileName,
                                char* buffer, Integer bufferLength,
                                Integer lineLength );

static SwathInfo* readSwathData( Data* data,
                                 const int file,
                                 const int swathIndex );
...
/******************************************************************************
PURPOSE: main - Read a subset of a BEHR file and write it to stdout in
         XDR or ASCII format.
...
static void printUsage( const char* programName ) {
  PRE0( programName );
  fprintf( stderr, "\a\n\n%s - Read a set of BEHR files and extract swath\n",
           programName );
  ...
  fprintf( stderr, "-variable AerosolOpticalDepth_at_550nm \\\n" );
  ...
  fprintf( stderr, "http://behr.cchem.berkeley.edu/TheBEHRProduct.aspx\n");
  ...
  fprintf( stderr, "Longitude Latitude Scan_Start_Time ColumnAmountNO2\n" );
        ...
        if ( parseTimestampAndHours( argc, argv, &arg,
          ...
          if ( arguments->variableName ) {
            ...
            if ( parseDomain( argc, argv, arguments ) ) {
              result = parseCorners( argc, argv, arguments );
            }
          }
        }
      }
...
/******************************************************************************
PURPOSE: readData - Read each BEHR file for selected time range and subset
...
static void readData( Data* data ) {
  ...
  Integer fileCount = 0;
  FileBounds* fileBounds = readListFile( arguments->listFile, &fileCount );
  data->ok = 0;

  if ( AND2( fileBounds, fileCount > 0 ) ) {
    VoidList* const swaths = newVoidList( deallocateSwathInfo, 0 );
    ...
    if ( swaths ) {
      ...
      for ( fileIndex = 0; fileIndex < fileCount; ++fileIndex ) {
        const char* const fileName = fileBounds[ fileIndex ].fileName;
        int file = openFile( fileName );

        if ( file != -1 ) {
          const int swathCount = swathsInFile( file );
          int swathIndex = 0;

          /*
           * For each swath in input data file:
           *   read, filter and subset data,
           *   write subset data to temporary file in tmpdir
           *   and store timestamp and number of subset points in swaths list.
           */

          for ( swathIndex = 0; swathIndex < swathCount; ++swathIndex ) {
            SwathInfo* swathInfo = readSwathData( data, file, swathIndex );
            data->ok = failureCount() == failures; /* Ignore bad files. UGLY.*/

            if ( AND2( data->ok, swathInfo ) ) {
              swaths->insert( swaths, swathInfo, LAST_ITEM );
            }

            if ( OR2( ! data->ok, ! swaths->ok( swaths ) ) ) {
              data->ok = 0;
              FREE( swathInfo );
              fileIndex = fileCount; /* Stop looping if failed allocation. */
            }
          }
  ...
  data->ok = AND2( data->ok, data->swaths->count( data->swaths ) );
...
/******************************************************************************
PURPOSE: readListFile - Read listFile containing list of BEHR files to read.
INPUTS:  const char* listFile   File containing list of BEHR data file names.
OUTPUTS: Integer*    fileCount  Number of files in bounds file.
RETURNS: FileBounds* Allocated array of FileBounds, else 0 if unsuccessful.
NOTES:   Returned result must be FREEd when no longer needed. LEAK.
         All of the files should be read/processed without checking for
         time/space subsets. This is the format and usage used by NASA SQL.
******************************************************************************/

static FileBounds* readListFile( const char* listFile, Integer* fileCount ) {

  PRE03( listFile, *listFile, fileCount );
  FileBounds* result = 0;
  Integer length = 0;
  char* buffer = 0;
  buffer = readFile( listFile, &length );
  *fileCount = 0;

  if ( buffer ) {
    *fileCount = convertNewlines( listFile, buffer, length, 0 );

    if ( *fileCount ) {
      result = NEW_ZERO( FileBounds, *fileCount );

      if ( result ) { /* Parse lines in buffer: */
        Integer file = 0;
        const char* line = buffer;

        do {
          FileBounds* const fileBounds = result + file;
          char fileName[ FILE_NAME_LENGTH ] = "";
          char format[ 80 ] = "%";
          memset( fileName, 0, sizeof fileName );
          CHECK2( line, *line );
          sprintf( format + 1, "%lus",
                   sizeof fileName / sizeof *fileName - 1 );
          CHECK( strlen( format ) < sizeof format / sizeof *format );

          if ( sscanf( line, format, fileName ) != 1 ) {
            failureMessage("Invalid listFile '%s' line '%s'.", listFile, line);
            file = *fileCount = 0;
          } else {
            strncpy( fileBounds->fileName, fileName, FILE_NAME_LENGTH - 1 );
            CHECK( strlen( fileBounds->fileName ) <
                   sizeof fileBounds->fileName /
                   sizeof fileBounds->fileName[ 0 ] );
            fileBounds->bounds[ LONGITUDE ][ MINIMUM ] = -180.0;
            fileBounds->bounds[ LONGITUDE ][ MAXIMUM ] =  180.0;
            fileBounds->bounds[ LATITUDE  ][ MINIMUM ] =  -90.0;
            fileBounds->bounds[ LATITUDE  ][ MAXIMUM ] =   90.0;
          }

          line += strlen( line ) + 1; /* Variable-length lines. */
  ...
  POST0( IMPLIES_ELSE( result, *fileCount > 0, *fileCount == 0 ) );
  return result;
}
...
/******************************************************************************
PURPOSE: convertNewlines - Convert and count newlines in bounds file string.
INPUTS:  const char* boundsFileName   Name of bounds file.
         char*    buffer        String contents of bounds file.
         Integer  bufferLength  Length of buffer.
         Integer  lineLength    If file contains lon-lat bounds, else 0.
RETURNS: Integer number of lines, else 0 if unsuccessful.
NOTES:   If lineLength is 0 then the bounds file contained no lon-lat bounds
         and all of the files should be read/processed without checking for
         time/space subsets. This is the format and usage used by NASA SQL.
******************************************************************************/
...
static Integer convertNewlines( const char* boundsFileName,
  ...
  PRE06( boundsFileName, *boundsFileName, buffer, *buffer,
         bufferLength > 0, lineLength >= 0 );
  Integer result = 0;
  char* line = buffer;

  if ( ! lineLength ) { /* NASA-format listFile: */
    char* const end = buffer + bufferLength;

    /* Convert newlines (at irregular indices) to nulls: */
    ...
    char* const end = buffer + bufferLength - lineLength;

    /* Convert newlines (at regular indices) to nulls: */

    for ( line = buffer; line < end; line += lineLength + 1 ) {

      if ( line[ lineLength ] != '\n' ) {
        failureMessage( "Invalid bounds file '%s'.", boundsFileName );
        line = end;
        result = 0;
      } else {
        line[ lineLength ] = '\0';
...
INPUTS:  Data*       data   data->arguments.
         ...
         const int   swath  0-based index of swath in file to read.
OUTPUTS: Data*       data   data->ok, data->units.
RETURNS: SwathInfo*  Initialized swath info from file, else 0 if unsuccessful.
******************************************************************************/

static SwathInfo* readSwathData( Data* data, const int file,
                                 const int swath ) {

  PRE06( data, data->ok, data->swaths,
         file > -1, swath >= 0,
         isValidArguments( &data->arguments ) );
  ...
  size_t rows = 0;
  size_t columns = 0;
  data->ok = readDimensions( file, swath, &rows, &columns );

  DEBUG( fprintf( stderr, "readSwathData ( file = %d, swath = %d ), "
                  "rows = %lu, columns = %lu\n",
                  file, swath, rows, columns ); )

  if ( data->ok ) {
    const size_t variables =
      3 + arguments->corners * 8; /* lon,lat,var + corners. */
    const size_t points = rows * columns;
    const size_t size = variables * points;
    ...
    if ( data->buffer ) {
      ...
      double* const longitudesSW = corners ? values   + points : 0;
      ...
      Name unused = "";
      data->ok = readDataset( file, swath, rows, columns,
                              "Longitude", unused, longitudes );

      if ( data->ok ) {
        data->ok = readDataset( file, swath, rows, columns,
                                "Latitude", unused, latitudes );

        if ( data->ok ) {
          data->ok = clampInvalidCoordinates( points, longitudes, latitudes );
          ...
          if ( data->ok ) {
            ...
            if ( data->ok ) {
              ...
              const double firstValue = values[ 0 ];
              const Integer timestamp = (Integer) firstValue;
              data->ok = IN_RANGE( timestamp, firstTimestamp, lastTimestamp );

              if ( data->ok ) {
                data->ok = readDataset( file, swath, rows, columns,
                                        arguments->variableName, data->units,
                                        values );
